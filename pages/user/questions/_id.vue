      ...
      >
        ...
        <div class="flex flex-col sm:flex-row w-full">
          <div class="flex w-full sm:w-1/2 flex-col justify-between">
              ...
              <div class="w-full py-4">
                <ul id="gallery" class="flex flex-1 flex-wrap -m-1">
                  <li
                    ...
                    v-bind:key="image"
                    class="block p-1 w-1/4 h-24 min-w-32"
                  >
                    <article
                      tabindex="0"
                      ...
                      >
                        ...
                        <div class="flex items-center">
                          <a v-bind:href="image" v-bind:download="image">
                            <div
                              class="ml-auto focus:outline-none hover:bg-gray-300 p-1 rounded-md"
                            >
                              <svg
                                ...
                                style="
                                  enable-background: new 0 0 395.638 395.638;
                                "
                                xml:space="preserve"
                                width="512px"
                                height="512px"
                  ...
                  <li
                    v-for="file in files"
                    v-bind:key="file"
                    class="block p-1 w-1/4 h-24"
                  >
                    <article
                      tabindex="0"
                      class="group hasImage w-full h-full rounded-md focus:outline-none focus:shadow-outline bg-gray-300 cursor-pointer relative text-transparent hover:text-white shadow-sm"
                    >
                      <img
                        v-bind:alt="file"
              ...
              >
                ...
                >
                  ...
                  >
                    ...
                    <div
                      class="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row justify-center items-center"
                    >
                      <div
                        @click="rate = 5"
                        v-bind:class="rate > 4 ? 'text-primary' : ''"
                      ...
                      <div
                        @click="rate = 4"
                        v-bind:class="rate > 3 ? 'text-primary' : ''"
                        class="text-4xl text-gray-400 cursor-pointer"
                      >
                        ★
                      </div>
                      <div
                        @click="rate = 3"
                        v-bind:class="rate > 2 ? 'text-primary' : ''"
                        class="text-4xl text-gray-400 cursor-pointer"
                      >
                        ★
                      </div>
                      <div
                        @click="rate = 2"
                        v-bind:class="rate > 1 ? 'text-primary' : ''"
                        class="text-4xl text-gray-400 cursor-pointer"
                      >
                        ★
                      </div>
                      <div
                        @click="rate = 1"
                        v-bind:class="rate > 0 ? 'text-primary' : ''"
                        class="text-4xl text-gray-400 cursor-pointer"
                      >
                        ★
                      </div>
                    ...
                    <div
                      class="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row"
                    >
                      <span
                      ...
                      >
                        <div
                          @click="openModalAccept = false"
                          class="inline-flex justify-center w-full rounded-md border border-red px-4 py-2 bg-white text-red leading-6 font-medium shadow-sm focus:outline-none focus:border-blue-300 focus:shadow-outline-blue transition ease-in-out duration-150 sm:text-sm sm:leading-5 cursor-pointer"
                        >
                          انصراف
                        </div>
                      </span>
            ...
            >
              <div
                @click="openModalAccept = true"
                class="flex justify-center items-center border border-primary bg-white w-full sm:w-1/2 text-primary font-bold p-4 rounded-md focus:outline-none focus:shadow-outline cursor-pointer my-1 sm:my-0"
              >
                تایید حل پرسش
              </div>
              <hr class="w-1" />
            ...
            <div class="flex flex-col py-6 px-10">
              ...
              <div class="flex flex-col w-full mt-4 rounded-md shadow-md">
                ...
                <div class="flex w-full p-4 justify-between bg-gray-300">
                  ...
                  <p>
                    {{
                      !item.category_name ? "تعیین نشده" : item.category_name
                    }}
                  </p>
                </div>
                <div class="flex w-full p-4 justify-between">
          ...
          <div class="flex flex-col w-full p-10">
            <div class="flex w-full">
              <div
                @click="tabOpener(1)"
                v-bind:class="
                  openTab === 1 ? 'text-primary border-b border-primary' : ''
                "
                class="p-3 px-6 font-bold cursor-pointer"
              >
                پیشنهادات
              </div>
              <div
                @click="tabOpener(2)"
                v-bind:class="
                  openTab === 2 ? 'text-primary border-b border-primary' : ''
                "
                class="p-3 px-6 font-bold cursor-pointer"
              >
                نظرات
              ...
              <div
                @click="tabOpener(3)"
                v-bind:class="
                  openTab === 3 ? 'text-primary border-b border-primary' : ''
                "
                class="p-3 px-6 font-bold cursor-pointer"
              >
                پاسخ ها
            ...
            <div class="flex w-full">
              <div class="flex w-full py-10" v-if="openTab === 1">
                <ul v-if="offers.length !== 0" class="w-full">
                  <li
                    ...
                    v-bind:key="offer.id"
                    class="flex flex-col sm:flex-row items-center justify-evenly w-full py-3 px-2 mb-2 rounded-md shadow-sm border"
                  >
                    <img
                      class="rounded-full w-12"
                      v-bind:src="offer.owner_profile_pic"
                      alt=""
                    />
                    <div
                      class="flex flex-col justify-center items-center px-3 py-2 sm:py-0"
                    >
                      <p class="text-sm font-bold">
                        {{ offer.owner_full_name }}
                      </p>
                      <p class="text-sm">
                        {{ offer.owner_current_grade }} -
                        {{ offer.owner_current_category }}
                      </p>
                    </div>
                    <div class="flex px-3 py-2 sm:py-0">
                      <p>قیمت پیشنهادی : &nbsp;</p>
                      <p>{{ offer.offered_cost_comma }} - تومان</p>
                    </div>
                    ...
                    <div class="px-3">
                      <div
                        @click="triggerModal(offer)"
                        class="rounded-md bg-primary text-white p-3 cursor-pointer"
                      >
                        پذیرش پیشنهاد
                      </div>
                    </div>
                ...
                <div v-else class="w-full">
                  <div
                    class="w-full flex flex-col justify-center items-center p-12"
                  >
                    <img
                      class="w-20"
              ...
              <div class="flex w-full pt-2 sm:p-10" v-else-if="openTab === 2">
                <div class="flex flex-col w-full justify-center items-center">
                  <Chat
                    v-bind:messages="comments"
                    v-bind:type="'comments'"
                    v-bind:question="this.$route.params.id"
                    v-bind:active="answers.length < 1"
                  />
        ...
        >
          ...
          >
            <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
              <div class="sm:flex sm:items-start">
                <div
                  class="w-full mt-3 text-center sm:mt-0 sm:ml-4 sm:text-right"
                >
                  <h3
            ...
            <div class="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row">
              ...
              >
                <div
                  @click="paymentByGate()"
                  class="inline-flex justify-center w-full rounded-md border border-primary px-4 py-2 bg-white text-primary leading-6 font-medium shadow-sm focus:outline-none focus:border-blue-300 focus:shadow-outline-blue transition ease-in-out duration-150 sm:text-sm sm:leading-5 cursor-pointer"
                >
                  پرداخت اینترنتی
                </div>
              </span>
...
export default {
  mounted: function () {
    ...
    var replaceDigits = function () {
      ...
      for (const e of elements) {
        e.innerHTML = e.innerHTML.replace(/\d(?=[^<>]*(<|$))/g, function ($0) {
          return map[$0];
        });
      }
    };
  ...
  async asyncData({ $axios, params, $auth }) {
    ...
    const offers = o_res.data.results.filter(
      (x) => x.question === q_res.data.id && !x.is_accepted
    );
    const FILES = [q_res.data.pic_1];
  ...
  methods: {
    ...
    async cancel() {
      const res = await this.$axios.get(
        `/api/questions/${this.$route.params.id}/cancel`
      );
    ...
    async accept() {
      ...
      const res = await this.$axios.post(
        `/api/questions/${this.$route.params.id}/acceptsolved/`,
        {
          rating: this.rate,
        }
      );
    ...
    async judgement() {
      const res = await this.$axios.get(
        `/api/questions/${this.$route.params.id}/judgereq/`
      );
      this.$router.push("/user/questions");
    },
  },
