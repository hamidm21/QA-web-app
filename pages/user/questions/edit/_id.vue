        ...
        <div>
          <div class="p-2 flex w-full justify-center">
            <form class="flex flex-col w-full sm:w-3/5 h-full" action="">
              ...
              <div
                class="flex flex-col sm:flex-row w-full p-2 justify-center items-center"
              >
                <div class="flex w-1/2 sm:w-1/4 justify-center items-center">
                  <div
                    @click="submitProject()"
                    class="flex justify-center items-center bg-primary w-full text-white font-bold p-2 m-4 rounded focus:outline-none focus:shadow-outline cursor-pointer shadow-md cursor-pointer"
                  >
                    ویرایش پروژه
                  </div>
                </div>
                <div class="flex w-1/2 sm:w-1/4 justify-center items-center">
                  <div
                    @click="cancel()"
                    class="flex justify-center items-center bg-red w-full text-white font-bold p-2 m-4 rounded focus:outline-none focus:shadow-outline cursor-pointer shadow-md cursor-pointer"
                  >
                    لغو
                  </div>
                </div>
...
export default {
  mounted: function () {
    ...
    if (!this.dd2Selected.category_less && this.categories.sub_cats) {
      this.dd3Handler(
        this.categories.filter((x) => x.id === this.item.category)[0]
      );
      this.dd4Selected = this.dd3Selected.sub_cats.filter(
        (x) => x.id === this.item.sub_category
      )[0];
    } else {
      this.dd4Selected = this.catless.filter(
        (x) => x.id === this.item.sub_category
      )[0];
    }
  },
