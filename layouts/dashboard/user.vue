<template>
  <div @click="closer($event)" class="user-layout">
    <div class="user-head flex shadow-md">
      ...
      >
        ...
        >
          <div class="flex justify-center items-center relative">
            ...
            >
              <div
                ...
                class="flex justify-center items-center bg-red absolute top-0 right-0 w-4 h-4 rounded-full text-white text-center text-xxs"
              >
            ...
            >
              <div class="rounded-md bg-white shadow-xs">
                <div class="py-1">
                  ...
                  <hr />
                  <div
                    @click="profSelect('/user/dashboard')"
                    class="flex items-center justify-start w-full px-4 py-2 text-sm leading-5 text-gray-700 hover:bg-gray-100 hover:text-gray-900 focus:outline-none focus:bg-gray-100 focus:text-gray-900"
                  >
                    <img
                      class="pl-2 w-6"
                  ...
                  <div
                    @click="profSelect('/user/questions')"
                    class="flex items-center justify-start w-full px-4 py-2 text-sm leading-5 text-gray-700 hover:bg-gray-100 hover:text-gray-900 focus:outline-none focus:bg-gray-100 focus:text-gray-900"
                  >
                    <img
                      class="pl-2 w-6"
                  ...
                  <div
                    @click="logout()"
                    class="flex text-red items-center justify-start w-full text-right px-4 py-2 text-sm leading-5 hover:bg-gray-200 focus:outline-none focus:bg-gray-100"
                  >
                    <img
                      class="pl-2 w-1/6"
          ...
          >
            <div
              v-show="isNotifOpen"
              class="origin-bottom-right absolute left-1-5 sm:left-8 top-5 sm:top-4 mt-2 w-64 rounded-md shadow-lg"
              style="direction: rtl"
            >
              <div
                v-if="this.$store.state.dashboard.notifs.length > 0"
                class="w-full rounded-md bg-gray-100 shadow-md p-2"
              >
                <div
                  @click="viewNotif(notif)"
                  v-for="notif in this.$store.state.dashboard.notifs"
                  v-bind:key="notif.id"
                  class="flex flex-col bg-white p-2 rounded-md shadow-md my-1 hover:shadow-lg cursor-pointer"
                >
                ...
                <div class="flex w-full justify-between items-center">
                  <div
                    class="flex justify-center items-center rounded-full w-8 h-8 cursor-pointer hover:shadow-md"
                  >
                    <img
                      @click="viewAll()"
                      class="w-5 my-2"
        ...
        <div
          @click="openMenu = true"
          class="absolute flex justify-center items-center right-0 sm:hidden mr-4 w-12 h-12 rounded-full hover:shadow-md cursor-pointer"
        >
          <img
            class="w-6 mx-4"
                  ...
                  <div>
                    <div class="flex flex-col w-full mt-3 text-center relative">
                      <div
                        ...
                        class="flex justify-center items-center w-12 h-12 cursor-pointer absolute"
                      >
                      ...
                      >
                        <div @click="menuSelect('/user/dashboard')">
                          <div class="flex">پیشخوان</div>
                        </div>
                        <hr class="w-full" />
                        <div @click="menuSelect('/user/questions')">
                          <div>پروژه های من</div>
                        </div>
                        <hr class="w-full" />
                        <div
                          v-show="$auth.user.is_teacher"
                          @click="menuSelect('/user/myTasks')"
                        >
                          <div>وظایف من</div>
                        </div>
                        <hr v-show="$auth.user.is_teacher" class="w-full" />
                        <div
                          v-show="$auth.user.is_judge"
                          @click="menuSelect('/user/judge')"
                        >
                          <div>داوری های من</div>
                        </div>
                        <hr v-show="$auth.user.is_judge" class="w-full" />
                        <div @click="menuSelect('/user/othersQuestions')">
                          <div>پروژه های دیگران</div>
                        </div>
                        <hr class="w-full" />
      ...
      <div
        class="side-nav sidn hidden sm:flex flex-col"
        style="
      ...
      >
        <div class="w-full h-fit flex justify-start items-center">
          <nuxt-link to="/">
            <img
              class="w-4/5 mb-4 mt-1"
              src="~/assets/icons/full_white.png"
              alt="لوگو حل یاب"
          /></nuxt-link>
        </div>
          ...
          <ul>
            <nuxt-link to="/user/dashboard">
              <li
                class="py-2 flex"
                v-bind:class="
                  this.$store.state.user.dashboard.page === 'dashboard'
                    ? 'bg-primedark rounded-md'
                    : ''
                "
              >
                <img
            ...
            <nuxt-link to="/user/questions">
              <li
                class="py-2 flex"
                v-bind:class="
                  this.$store.state.user.dashboard.page === 'questions'
                    ? 'bg-primedark rounded-md'
                    : ''
                "
              >
            ...
            <nuxt-link v-show="$auth.user.is_teacher" to="/user/myTasks">
              <li
                class="py-2 flex"
                v-bind:class="
                  this.$store.state.user.dashboard.page === 'myTasks'
                    ? 'bg-primedark rounded-md'
                    : ''
                "
              >
                <img
                  class="w-5 mx-2"
                  src="~/assets/icons/icon-briefcase.svg"
                  alt="وظایف من"
            ...
            <nuxt-link to="/user/othersQuestions">
              <li
                class="py-2 flex"
                v-bind:class="
                  this.$store.state.user.dashboard.page === 'others'
                    ? 'bg-primedark rounded-md'
                    : ''
                "
              >
            ...
            <nuxt-link v-show="$auth.user.is_judge" to="/user/judge">
              <li
                class="py-2 flex"
                v-bind:class="
                  this.$store.state.user.dashboard.page === 'judge'
                    ? 'bg-primedark rounded-md'
                    : ''
                "
              >
            ...
            <nuxt-link to="/user/profile">
              <li
                class="py-2 flex"
                v-bind:class="
                  this.$store.state.user.dashboard.page === 'profile'
                    ? 'bg-primedark rounded-md'
                    : ''
                "
              >
            ...
            <nuxt-link to="/user/trx">
              <li
                class="py-2 flex"
                v-bind:class="
                  this.$store.state.user.dashboard.page === 'finance'
                    ? 'bg-primedark rounded-md'
                    : ''
                "
              >
            ...
            <nuxt-link to="/user/teachers">
              <li
                class="py-2 flex"
                v-bind:class="
                  this.$store.state.user.dashboard.page === 'teachers'
                    ? 'bg-primedark rounded-md'
                    : ''
                "
              >
                <img
                  class="w-5 mx-1"
                  src="~/assets/icons/icon-user-check.svg"
                  alt="اساتید حل یاب"
                  style="filter: invert(0.7)"
                />
                اساتید حل یاب
            ...
            <nuxt-link to="/contact_us">
              <li
                class="py-2 flex"
                v-bind:class="
                  this.$store.state.user.dashboard.page === 'contactUs'
                    ? 'bg-primedark rounded-md'
                    : ''
                "
              >
            ...
            <nuxt-link to="/about_us">
              <li
                class="py-2 flex"
                v-bind:class="
                  this.$store.state.user.dashboard.page === 'aboutUs'
                    ? 'bg-primedark rounded-md'
                    : ''
                "
              >
...
export default {
  ...
  methods: {
    ...
    async viewNotif(notif) {
      ...
      this.$store.commit(
        "setUnReadNotifs",
        this.$store.state.dashboard.unReads - 1
      );
